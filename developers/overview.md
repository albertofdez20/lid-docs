...
This section is intentionally high-level during the alpha phase. Full technical reference, contract ABIs, and endpoint specs will publish when the API surface stabilizes post-raise.
...
- **Agent builders** who are thinking about how autonomous software actors will transact.
...
- **Product metadata:** On-chain + metadata storage layer
- **Checkout UX:** Hosted by LID, fully abstracted (no wallet exposure for end users)
- **Developer surface (today):** Web app
- **Developer surface (roadmap):** Agentic Commerce API + SDK

## Why Solana

Three properties were required, and only Solana delivered all three at the scale LID needs:
...
- **Build on LID:** email alberto@lid.pro to get into the integration waitlist.
